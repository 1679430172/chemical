...
public interface OrderMapper extends BaseMapper<Order> {
    ...
    @Select("select * from `order` o inner join commodity c on o.commodity_id=c.sid where o.user_id=#{userId} and status = 1")
    public IPage<Order> selectListByStatus(@Param("userId") Integer userId,Page page);

    @Results({
            @Result(column = "name",property = "name")
    })
    @Select("select * from `order` o inner join commodity c on o.commodity_id=c.sid where status = 1")
