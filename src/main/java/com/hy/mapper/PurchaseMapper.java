...
public interface PurchaseMapper extends BaseMapper<Purchase> {
    @Select("select cid,user_id,bh,name,cas,amount,price,price_status,sum_price,status,supplier_name,supplier_phone,lxr,tracking_number,create_time,ann,zt,b.user_name  from purchase a,users b where a.user_id=b.uid and ann=1 UNION all select cid,user_id,bh,name,cas,amount,price,price_status,sum_price,status,supplier_name,supplier_phone,lxr,tracking_number,create_time,ann,zt,b.user_name  from purchase a,users b where a.user_id=b.uid and ann in(2,3) ORDER BY ann asc,create_time desc")
    ...
    @Update("update purchase set zt = 2 where cid=#{cid}")
    ...
    @Insert("insert into purchase(cid,user_id,bh,name,cas,amount,price,price_status,sum_price,status,user_name,supplier_name,supplier_phone,lxr,tracking_number,create_time,ann,zt) values(#{cid},#{userId},#{bh},#{name},#{cas},#{amount},#{price},#{priceStatus},#{sumPrice},#{status},#{userName},#{supplierName},#{supplierPhone},#{lxr},#{trackingNumber},now(),1,1)")
